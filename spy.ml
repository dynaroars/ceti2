...
type spy_t = CM.sid_t list*int*int*int list (*sid,cid,level,idxs*)
...
class mtempl_VS cname cid level = object(self)
  ...
  method mkInstr (ast:file) (mainFd:fundec) (sid:int)
    ...
    E.log "** xinfo %s, |vs|=%d, [%s]\n" xinfo nvs
	  (CM.string_of_list (L.map (fun vi -> vi.vname) vs));
...
let spy 
  ...
  |Instr ins ->
    ...
    let rs = L.map(fun cl -> spyf (cl#level <= tplLevel) cl) tplCls in
    CM.list_of_some rs
...
let () = begin
    ...
    let rs = spy ast.fileName stmtHt sid tplLevel maxV in
    ()
